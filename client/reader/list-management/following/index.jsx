// External dependencies
import React from 'react';
import times from 'lodash/utility/times';
//import debugModule from 'debug';

// Internal dependencies
import Main from 'components/main';
import Navigation from 'reader/list-management/navigation';
import InfiniteList from 'components/infinite-list';
import ListItem from 'reader/list-item';
import Icon from 'reader/list-item/icon';
import Title from 'reader/list-item/title';
import Description from 'reader/list-item/description';
import Actions from 'reader/list-item/actions';
import SiteIcon from 'components/site-icon';
import FollowButton from 'components/follow-button';

//const debug = debugModule( 'calypso:reader:list-management' );

const ListManagementFollowing = React.createClass( {
	propTypes: {
		list: React.PropTypes.shape( {
			owner: React.PropTypes.string.isRequired,
			slug: React.PropTypes.string.isRequired
		} )
	},

	getInitialState() {
		const following = this.getFollowing();
		let fetching = false;
		if ( following.length === 0 ) {
			//fetchMore();
			fetching = true;
		}
		return {
			following,
			fetching,
			page: 1
		};
	},

	getFollowing() {
		const following = {}; //ReaderListContents.get();
		return following;
		// return contents.map( function( rec ) {
		// 	rec.site = SiteStore.get( rec.blog_id );
		// 	return rec;
		// } );
	},

	update() {
		this.setState( { following: this.getFollowing() } );
	},

	componentDidMount() {
		// SiteStore.on( 'change', this.update );
		// RecommendedSites.on( 'change', this.update );
		// RecommendedSites.on( 'change', this.stopFetching );
	},

	componentWillUnmount() {
		// SiteStore.off( 'change', this.update );
		// RecommendedSites.off( 'change', this.update );
		// RecommendedSites.off( 'change', this.stopFetching );
	},

	loadMore( options ) {
		//fetchMore(); // in store actions
		this.setState( { fetching: true } );
		// if ( options.triggeredByScroll ) {
		// 	this.props.trackScrollPage( RecommendedSites.getPage() );
		// }
	},

	stopFetching() {
		this.setState( {
			fetching: false,
			page: this.state.page + 1
		} );
	},

	renderPlaceholders() {
		const placeholders = [],
			number = this.state.following.length ? 2 : 10;

		times( number, ( i ) => {
			placeholders.push(
				<ListItem className="is-placeholder" key={'list-placeholder-' + i}>
					<Icon><SiteIcon size={48} /></Icon>
					<Title>Loading</Title>
					<Description>Loading the results...</Description>
				</ListItem>
			);
		} );

		return placeholders;
	},

	getItemRef( item ) {
		return 'list-' + rec.blog_id;
	},

	trackFollowToggle( following ) {
		if ( following ) {
			recordFollow();
		} else {
			recordUnfollow();
		}
	},

	trackSiteClick() {
		stats.recordAction( 'click_site_on_list_following' );
		stats.recordGaEvent( 'Clicked Site on List Following' );
	},

	renderItem( rec ) {
		// const site = rec.site && rec.site.toJS(),
		// 	itemKey = this.getItemRef( rec ),
		// 	title = site.name || ( site.URL && url.parse( site.URL ).hostname );

		return (
			<ListItem key={ itemKey } ref={ itemKey }>
				<Icon><SiteIcon site={ site } size={ 48 } /></Icon>
				<Title>
					<a href={'/read/blog/id/' + site.ID} onclick={ this.trackSiteClick }>{ title }</a>
				</Title>
				<Description>{ rec.reason }</Description>
				<Actions>
					<FollowButton siteUrl={ site.URL } onFollowToggle={ this.trackFollowToggle } />
				</Actions>
			</ListItem>
		);
	},

	render() {
		return (
			<Main className="list-management-following">
				<Navigation selected="following" list={ this.props.list } />
				<InfiniteList
					items={ this.state.following }
					fetchingNextPage={ this.state.fetching }
					lastPage={ this.state.following && this.state.following.length >= 100 }
					guessedItemHeight={ 300 }
					fetchNextPage={ this.loadMore }
					getItemRef={ this.getItemRef }
					renderItem={ this.renderItem }
					renderLoadingPlaceholders={ this.renderPlaceholders }
				/>
			</Main>
			);
	}
} );

export default ListManagementFollowing;
