...
module.exports = React.createClass( {
	...
	getDisabledInfo: function() {
		...
		if ( this.props.site.options.unmapped_url !== this.props.site.options.main_network_site ) {
			return this.translate( 'Only the main site on a multi-site installation can remove plugins.', {
				args: { site: this.props.site.title }
			} );
		}
		...
		if ( ! this.props.site.canUpdateFiles && this.props.site.options.file_mod_disabled ) {
			...
			if ( reasons.length > 1 ) {
				html.push(
					<p key="reason-shell">
						{ this.translate( 'Removing plugins is not available for %(site)s:', { args: { site: this.props.site.title } } ) }
					</p>
				);
				...
				html.push(
					<p key="reason-shell">{
						this.translate( 'Removing plugins is not available for %(site)s. %(reason)s', {
